...
import { handleDimMouseDown, onMouseMove, drawPreviewDimension } from './DimensionTool.js'
import { handleEditMouseDown, handleEditMouseMove, handleEditMouseUp, isEditCommand, drawEditPreview } from './EditTools.js'
import { TOOL_KEYS, ACTION_KEYS } from '../../config/keybindings.js'
import { GRID_SIZE, ARROW_SIZE, SNAP_INDICATOR_RADIUS_INNER, SNAP_INDICATOR_RADIUS_OUTER } from '../../config/constants.js'
...
class Engine {
  ...
  _hitTest(wx, wy) {
    ...
    for (let i = entities.length - 1; i >= 0; i--) {
      ...
      if (e.type === 'LINE') {
        ...
        if (e.dimType === 'linear') {
          const dx = e.x2 - e.x1, dy = e.y2 - e.y1; const len = Math.hypot(dx, dy)
          if (len > 0.01) {
            const nx = -dy / len, ny = dx / len; const off = e.offset || 15
          ...
          const { cx, cy } = e; const radius = e.radius || 0
          const la = e.leaderAngle !== undefined ? e.leaderAngle : -Math.PI / 4; const ext = 15
          const ex = cx + radius * Math.cos(la); const ey = cy + radius * Math.sin(la)
          const tx = cx + (radius + ext) * Math.cos(la); const ty = cy + (radius + ext) * Math.sin(la)
          if (ptSegDist(wx, wy, ex, ey, tx, ty) < th) return e
        } else if (e.dimType === 'angle') {
          const { vertexX, vertexY, angleStart, angleEnd } = e; const ar = e.arcRadius || 20
          ...
          if (Math.abs(d - ar) < th) {
            ...
            if (sa <= ea) { if (aa >= sa && aa <= ea) return e } else { if (aa >= sa || aa <= ea) return e }
          }
        }
      }
    }
  ...
  _makeRemoveCmd(entity) {
    ...
    return {
      ...
      undo() { if (idx >= 0) { self._entities.splice(idx, 0, entity); self._invalidateSnapshot() } }
    }
  }

  _makeMoveCmd(entity, before) {
    const self = this; const b = deepClone(before)
    return {
  ...
  _getSnappedPos(sx, sy) {
    ...
    if (!this._snap.snapToGrid && !this._snap.snapToEndpoint &&
        !this._snap.snapToMidpoint && !this._snap.snapToIntersection && !this._ortho) return world
  ...
  _logCommand(type, entityId, before, after) {
    if (this._onCommand) this._onCommand(type, entityId, before, after)
  }
  ...
  _setupSize() {
    ...
    this.snapshotCanvas.width = w; this.snapshotCanvas.height = h
  ...
  _createOverlays() {
    ...
    inp.type = 'text'; inp.placeholder = '輸入文字後 Enter 確認'
    ...
    ti.appendChild(inp); document.body.appendChild(ti)
    this._textInputOverlay = ti; this._textInput = inp
    inp.addEventListener('keydown', this._onTextInputKeyDown)

    const ph = document.createElement('div')
    ph.id = '__engine_polygon_hint'; ph.textContent = '雙擊完成多邊形 • Enter 閉合 • Esc 取消'
    ...
    document.body.appendChild(ph); this._polygonHintEl = ph

    const plh = document.createElement('div')
    plh.id = '__engine_polyline_hint'; plh.textContent = '點按新增頂點 • Enter 閉合 • Esc 取消 • 雙擊結束'
    ...
    document.body.appendChild(plh); this._polylineHintEl = plh
  }
  ...
  _removeOverlays() {
    ...
    if (this._polygonHintEl && this._polygonHintEl.parentNode) this._polygonHintEl.parentNode.removeChild(this._polygonHintEl)
    if (this._polylineHintEl && this._polylineHintEl.parentNode) this._polylineHintEl.parentNode.removeChild(this._polylineHintEl)
    this._textInputOverlay = null; this._textInput = null; this._polygonHintEl = null; this._polylineHintEl = null
  }
  ...
  _finishPolygon() {
    ...
    if (e && e.vertices.length >= 3) {
      ...
      this._emit('entityAdded', added); this._emit('modified')
    }
    this._isDrawing = false; this._drawingEntity = null; this._drawingPhase = 0
  ...
  _finishPolyline(close) {
    ...
    if (e && e.vertices.length >= 2) {
      const cmd = this._makeAddCmd(this._makeEntity('POLYLINE', {
        vertices: e.vertices.map(v => ({ x: v.x, y: v.y })), isClosed: !!close
      }))
      this._history.execute(cmd)
      const added = this._entities[this._entities.length - 1]
      this._logCommand('ADD_ENTITY', added.id, null, { ...added })
  ...
  _onMouseDown(e) {
    ...
    this._mouseScreenX = sx; this._mouseScreenY = sy
    if (this._ortho && !this._isPanning) this._snapOrigin = { x: world.x, y: world.y }

    if (this._tool === 'pan' || (e.button === 1 && this._tool === 'select')) {
      this._isPanning = true; this._panStart = { x: sx, y: sy }; this._panOffsetStart = { x: this._panX, y: this._panY }
    ...
    if (this._tool === 'select') {
      ...
      if (hit) {
        this._selectedEntity = hit; this._isDragging = true
        this._dragStart = { x: world.x, y: world.y }; this._dragBeforeState = JSON.parse(JSON.stringify(hit))
        this._invalidateSnapshot(); this._emit('select', hit); this.render()
      } else { this._selectedEntity = null; this._emit('select', null); this.render() }
      return
    }
    ...
    if (this._tool === 'polygon') {
      if (!this._isDrawing) {
        ...
        this._drawingPhase = 1; if (this._polygonHintEl) this._polygonHintEl.style.display = 'block'
      } else { this._drawingEntity.vertices.push({ x: world.x, y: world.y }) }
      this.render(); return
    }
    if (this._tool === 'polyline') {
      if (!this._isDrawing) {
        ...
        this._drawingPhase = 1; if (this._polylineHintEl) this._polylineHintEl.style.display = 'block'
      } else { this._drawingEntity.vertices.push({ x: world.x, y: world.y }) }
      this.render(); return
    }
    if (this._tool === 'text') {
      const ti = this._textInputOverlay; const inp = this._textInput
      if (ti && inp) {
        ti.style.display = 'block'; ti.style.left = (e.clientX + 10) + 'px'; ti.style.top = (e.clientY + 10) + 'px'
        inp.value = ''; inp._worldX = world.x; inp._worldY = world.y; inp.focus()
      }
      return
    }
    if (this._tool === 'dim') { handleDimMouseDown(this, world.x, world.y); return }
    if (isEditCommand(this._tool)) { handleEditMouseDown(this, world.x, world.y, this._tool); return }
  }
  ...
  _onMouseMove(e) {
    ...
    if (this._isPanning && this._panStart) {
      ...
      this._invalidateSnapshot(); this.render(); return
    }

    if (this._isDragging && this._selectedEntity && this._dragStart) {
      const dx = world.x - this._dragStart.x; const dy = world.y - this._dragStart.y
      this._moveEntity(this._selectedEntity, dx, dy)
      this._dragStart = { x: world.x, y: world.y }
      this._invalidateSnapshot(); this._emit('select', this._selectedEntity); this.render(); return
    }

    if (this._isDrawing && this._drawingEntity) {
      const de = this._drawingEntity
      if (de.type === 'LINE') { de.x2 = world.x; de.y2 = world.y }
      else if (de.type === 'CIRCLE') { de.radius = Math.hypot(world.x - de.cx, world.y - de.cy) }
      else if (de.type === 'ARC') {
    ...
    if (this._tool === 'dim') { if (onMouseMove(this, world)) return }
    if (isEditCommand(this._tool)) { if (handleEditMouseMove(this, world.x, world.y, this._tool)) return }
  ...
  _onMouseUp(e) {
    ...
    if (this._isDragging) {
      ...
      this._dragBeforeState = null; return
    }
    ...
    if (isEditCommand(this._tool)) { handleEditMouseUp(this, world.x, world.y, this._tool) }
  }
  ...
  _onDblClick(e) {
    ...
    if (this._tool === 'polygon' && this._isDrawing && this._drawingEntity) {
      ...
      if (verts.length >= 3) { verts.pop(); this._finishPolygon() }
    }
    if (this._tool === 'polyline' && this._isDrawing && this._drawingEntity) {
      const verts = this._drawingEntity.vertices
      if (verts.length >= 2) { this._finishPolyline(false) }
    }
  }

  _onTextInputKeyDown(e) {
    if (e.key === 'Enter') {
      const inp = this._textInput; const text = inp.value.trim()
      if (text && inp._worldX !== undefined) {
        ...
        const added = this._entities[this._entities.length - 1]
        this._logCommand('ADD_ENTITY', added.id, null, { ...added })
        this._emit('entityAdded', added); this._emit('modified'); this.render()
      }
      this._textInputOverlay.style.display = 'none'; inp.value = ''; this.setTool('select')
    } else if (e.key === 'Escape') {
      this._textInputOverlay.style.display = 'none'; this._textInput.value = ''; this.setTool('select')
    }
  }
  ...
  _onKeyDown(e) {
    ...
    if (this._textInputOverlay && this._textInputOverlay.style.display !== 'none') {
      if (e.key === 'Escape') { this._textInputOverlay.style.display = 'none'; this._textInput.value = ''; this.setTool('select') }
      return
    }
    ...
    if (e.ctrlKey || e.metaKey) {
      ...
      if (action === 'UNDO') { e.preventDefault(); if (e.shiftKey) { this.redo() } else { this.undo() }; this.render(); return }
      if (action === 'REDO') { e.preventDefault(); this.redo(); this.render(); return }
      return
    }

    if (e.target.closest('input') || e.target.closest('textarea')) return

    if (e.shiftKey && key === 'm') { this.setTool('mirror'); return }

    const toolName = TOOL_KEYS[key]
    if (toolName) { this.setTool(toolName.toLowerCase()); return }

    if (key === 's') { this.toggleSnap(); return }

    if (key === 'Delete' || key === 'Backspace') {
      if (this._selectedEntity && !e.target.closest('input')) {
        const before = { ...this._selectedEntity }; const removedId = this._selectedEntity.id
        const cmd = this._makeRemoveCmd(this._selectedEntity)
        this._history.execute(cmd)
        this._logCommand('DELETE_ENTITY', removedId, before, null)
        this._emit('entityRemoved', removedId); this._emit('modified'); this.render()
      }
      return
    }

    if (key === 'Escape') {
      if (this._dimState) { this._dimState = null; this._dimPreview = null; this._dimLine1 = null; this.render(); return }
      if (this._isDrawing && this._drawingEntity && (this._tool === 'polygon' || this._tool === 'polyline')) {
        this._isDrawing = false; this._drawingEntity = null; this._drawingPhase = 0
        if (this._polygonHintEl) this._polygonHintEl.style.display = 'none'
        if (this._polylineHintEl) this._polylineHintEl.style.display = 'none'
        this.render(); return
      }
      this.setTool('select'); return
    }

    if (key === 'Enter' && this._tool === 'polygon' && this._isDrawing && this._drawingEntity && this._drawingEntity.vertices.length >= 3) { this._finishPolygon(); return }
    if (key === 'Enter' && this._tool === 'polyline' && this._isDrawing && this._drawingEntity && this._drawingEntity.vertices.length >= 2) { this._finishPolyline(true); return }
  }
  ...
  _bindEvents() {
    ...
    this._onResize = () => { this._setupSize(); this.render() }
  ...
  _drawGrid(ctx) {
    ...
    const g = this._gridSize; const w = this.canvas.width; const h = this.canvas.height
    ctx.save()
    ctx.lineWidth = 0.5
    ctx.strokeStyle = '#2a2a3a'; ctx.beginPath()
    const ox = this._panX % (g * 5 * this._zoom); const oy = this._panY % (g * 5 * this._zoom)
    ...
    ctx.stroke()
    ctx.strokeStyle = '#222236'; ctx.beginPath()
    ...
    if (this._panX >= 0 && this._panX < w && this._panY >= 0 && this._panY < h) {
      ctx.strokeStyle = '#585b70'; ctx.lineWidth = 1.5; ctx.beginPath()
      ctx.moveTo(this._panX, 0); ctx.lineTo(this._panX, h)
      ctx.moveTo(0, this._panY); ctx.lineTo(w, this._panY); ctx.stroke()
    }
    ctx.restore()
  }
  ...
  _drawEntity(ctx, entity, highlight) {
    ...
    ctx.lineCap = 'round'
    if (highlight) {
      ctx.shadowColor = entity.color; ctx.shadowBlur = 12
      ctx.strokeStyle = '#ffffff'; ctx.lineWidth = (entity.lineWidth || 2) * this._zoom * 1.3
    }
    if (entity.type === 'LINE') {
      const s1 = this._worldToScreen(entity.x1, entity.y1); const s2 = this._worldToScreen(entity.x2, entity.y2)
      ctx.beginPath(); ctx.moveTo(s1.x, s1.y); ctx.lineTo(s2.x, s2.y); ctx.stroke()
    } else if (entity.type === 'CIRCLE') {
      const s = this._worldToScreen(entity.cx, entity.cy); const r = entity.radius * this._zoom * this._gridSize
      ctx.beginPath(); ctx.arc(s.x, s.y, r, 0, Math.PI * 2); ctx.stroke()
      if (!highlight) { ctx.fillStyle = entity.color; ctx.beginPath(); ctx.arc(s.x, s.y, 3 * this._zoom, 0, Math.PI * 2); ctx.fill() }
      ...
      ctx.beginPath(); const p0 = this._worldToScreen(verts[0].x, verts[0].y); ctx.moveTo(p0.x, p0.y)
      for (let i = 1; i < verts.length; i++) { const p = this._worldToScreen(verts[i].x, verts[i].y); ctx.lineTo(p.x, p.y) }
      ctx.closePath(); ctx.stroke()
      if (!highlight) {
        ctx.fillStyle = entity.color; ctx.globalAlpha = 0.08; ctx.fill(); ctx.globalAlpha = 1
        for (const v of verts) { const p = this._worldToScreen(v.x, v.y); ctx.beginPath(); ctx.arc(p.x, p.y, 3 * this._zoom, 0, Math.PI * 2); ctx.fill() }
      }
      ...
      ctx.beginPath(); const p0 = this._worldToScreen(verts[0].x, verts[0].y); ctx.moveTo(p0.x, p0.y)
      for (let i = 1; i < verts.length; i++) { const p = this._worldToScreen(verts[i].x, verts[i].y); ctx.lineTo(p.x, p.y) }
      if (entity.isClosed) ctx.closePath(); ctx.stroke()
      if (!highlight) { for (const v of verts) { const p = this._worldToScreen(v.x, v.y); ctx.beginPath(); ctx.arc(p.x, p.y, 3 * this._zoom, 0, Math.PI * 2); ctx.fill() } }
      ...
      if (entity.text) {
        ...
        ctx.fillStyle = highlight ? '#ffffff' : entity.color; ctx.textBaseline = 'bottom'
        ctx.fillText(entity.text, p.x, p.y)
      }
    } else if (entity.type === 'DIMENSION') {
      const dimColor = highlight ? '#ffffff' : (entity.color || '#a6e3a1')
      if (entity.dimType === 'linear') {
        const { x1, y1, x2, y2 } = entity; const offset = entity.offset || 15
        const s1 = this._worldToScreen(x1, y1); const s2 = this._worldToScreen(x2, y2)
        const dx = s2.x - s1.x, dy = s2.y - s1.y; const len = Math.hypot(dx, dy)
        if (len > 0.01) {
          const nx = -dy / len, ny = dx / len; const ox = nx * offset, oy = ny * offset
          const lx1 = s1.x + ox, ly1 = s1.y + oy; const lx2 = s2.x + ox, ly2 = s2.y + oy
          ctx.strokeStyle = dimColor; ctx.fillStyle = dimColor; ctx.lineWidth = 1.5 * this._zoom
          ctx.setLineDash([4 * this._zoom, 4 * this._zoom])
          ctx.beginPath(); ctx.moveTo(s1.x, s1.y); ctx.lineTo(lx1, ly1); ctx.moveTo(s2.x, s2.y); ctx.lineTo(lx2, ly2); ctx.stroke()
          ctx.setLineDash([]); ctx.lineWidth = 1.5 * this._zoom
          ctx.beginPath(); ctx.moveTo(lx1, ly1); ctx.lineTo(lx2, ly2); ctx.stroke()
          const arrowSize = ARROW_SIZE * this._zoom; const a1 = Math.atan2(ly2 - ly1, lx2 - lx1)
          ctx.beginPath(); ctx.moveTo(lx1, ly1); ctx.lineTo(lx1 - arrowSize * Math.cos(a1 - 0.4), ly1 - arrowSize * Math.sin(a1 - 0.4)); ctx.lineTo(lx1 - arrowSize * Math.cos(a1 + 0.4), ly1 - arrowSize * Math.sin(a1 + 0.4)); ctx.closePath(); ctx.fill()
          const a2 = Math.atan2(ly1 - ly2, lx1 - lx2)
          ctx.beginPath(); ctx.moveTo(lx2, ly2); ctx.lineTo(lx2 - arrowSize * Math.cos(a2 - 0.4), ly2 - arrowSize * Math.sin(a2 - 0.4)); ctx.lineTo(lx2 - arrowSize * Math.cos(a2 + 0.4), ly2 - arrowSize * Math.sin(a2 + 0.4)); ctx.closePath(); ctx.fill()
          const midX = (lx1 + lx2) / 2, midY = (ly1 + ly2) / 2
          const worldLen = Math.hypot(x2 - x1, y2 - y1)
          ctx.font = (12 * this._zoom) + 'px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'bottom'
          ctx.fillText(entity.text || String(Math.round(worldLen * 10) / 10), midX, midY - 4 * this._zoom)
        }
      } else if (entity.dimType === 'radius') {
        const { cx, cy } = entity; const radius = entity.radius || 0
        const leaderAngle = entity.leaderAngle !== undefined ? entity.leaderAngle : -Math.PI / 4
        const s = this._worldToScreen(cx, cy); const r = radius * this._zoom * this._gridSize
        const ex = s.x + r * Math.cos(leaderAngle); const ey = s.y + r * Math.sin(leaderAngle)
        const ext = 15 * this._zoom; const tx = s.x + (r + ext) * Math.cos(leaderAngle); const ty = s.y + (r + ext) * Math.sin(leaderAngle)
        ctx.strokeStyle = dimColor; ctx.fillStyle = dimColor; ctx.lineWidth = 1.5 * this._zoom
        ctx.beginPath(); ctx.moveTo(ex, ey); ctx.lineTo(tx, ty); ctx.stroke()
        const a = Math.atan2(ty - ey, tx - ex); const arrowSize = ARROW_SIZE * this._zoom
        ctx.beginPath(); ctx.moveTo(ex, ey); ctx.lineTo(ex + arrowSize * Math.cos(a + 2.7), ey + arrowSize * Math.sin(a + 2.7)); ctx.lineTo(ex + arrowSize * Math.cos(a - 2.7), ey + arrowSize * Math.sin(a - 2.7)); ctx.closePath(); ctx.fill()
        ctx.font = (12 * this._zoom) + 'px sans-serif'; ctx.textAlign = 'left'; ctx.textBaseline = 'bottom'
        ctx.fillText(entity.text || ('R=' + Math.round(radius * 10) / 10), tx + 4 * this._zoom, ty)
      } else if (entity.dimType === 'angle') {
        const { vertexX, vertexY, angleStart, angleEnd } = entity
        const arcRadius = entity.arcRadius || 20; const s = this._worldToScreen(vertexX, vertexY)
        const r = arcRadius * this._zoom * this._gridSize
        const start = Math.min(angleStart, angleEnd); const end = Math.max(angleStart, angleEnd)
        ctx.strokeStyle = dimColor; ctx.fillStyle = dimColor; ctx.lineWidth = 1.5 * this._zoom
        ctx.beginPath(); ctx.arc(s.x, s.y, r, start, end); ctx.stroke()
        const arrowSize = ARROW_SIZE * this._zoom
        for (const angle of [start, end]) {
          const ax = s.x + r * Math.cos(angle); const ay = s.y + r * Math.sin(angle); const perpAngle = angle + Math.PI / 2
          ctx.beginPath(); ctx.moveTo(ax, ay); ctx.lineTo(ax + arrowSize * Math.cos(perpAngle + 0.5), ay + arrowSize * Math.sin(perpAngle + 0.5)); ctx.lineTo(ax - arrowSize * Math.cos(perpAngle - 0.5), ay - arrowSize * Math.sin(perpAngle - 0.5)); ctx.closePath(); ctx.fill()
        }
        const midAngle = (start + end) / 2; const textR = r + 16 * this._zoom
        const tx = s.x + textR * Math.cos(midAngle); const ty = s.y + textR * Math.sin(midAngle)
        const angleDeg = Math.round((end - start) * 180 / Math.PI * 10) / 10
        ctx.font = (12 * this._zoom) + 'px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'
        ctx.fillText(entity.text || (angleDeg + '\u00b0'), tx, ty)
      }
    }
    ctx.restore()
  }
  ...
  _drawSelectionBox(ctx, entity) {
    ...
    if (entity.type === 'LINE') {
      ...
      const s = this._worldToScreen(entity.cx, entity.cy); const r = (entity.radius || 0) * this._zoom * this._gridSize
      b = { l: s.x - r, t: s.y - r, r: s.x + r, btm: s.y + r }
    } else if (entity.type === 'POLYGON' || entity.type === 'POLYLINE') {
      const verts = entity.vertices; if (verts.length === 0) { ctx.restore(); return }
      let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity
      for (const v of verts) { const p = this._worldToScreen(v.x, v.y); if (p.x < minX) minX = p.x; if (p.y < minY) minY = p.y; if (p.x > maxX) maxX = p.x; if (p.y > maxY) maxY = p.y }
      b = { l: minX, t: minY, r: maxX, btm: maxY }
    } else if (entity.type === 'TEXT') {
      const p = this._worldToScreen(entity.x, entity.y); const fs = (entity.fontSize || 1) * this._zoom * this._gridSize
      ...
      if (entity.dimType === 'linear') {
        ...
        const s = this._worldToScreen(entity.vertexX, entity.vertexY); const r = (entity.arcRadius || 20) * this._zoom * this._gridSize
        b = { l: s.x - r, t: s.y - r, r: s.x + r, btm: s.y + r }
      }
    }
    ...
    if (b) {
      const corners = [
        { x: b.l - 8, y: b.t - 8 }, { x: b.r + 8, y: b.t - 8 },
        { x: b.l - 8, y: b.btm + 8 }, { x: b.r + 8, y: b.btm + 8 }
      ]
  ...
  _renderSnapshot() {
    const w = this.canvas.width; const h = this.canvas.height; const ctx = this.snapshotCtx
  ...
  render() {
    if (!this._snapshotValid) { this._renderSnapshot(); this._snapshotValid = true }
    const ctx = this.ctx
    ...
    if (this._isDrawing && this._drawingEntity) {
      ...
      if (this._drawingEntity.type === 'POLYGON' && this._drawingEntity.vertices) {
        for (const v of this._drawingEntity.vertices) {
          const p = this._worldToScreen(v.x, v.y); ctx.fillStyle = '#a6e3a1'
      ...
      if (this._drawingEntity.type === 'POLYLINE' && this._drawingEntity.vertices) {
        for (const v of this._drawingEntity.vertices) {
          const p = this._worldToScreen(v.x, v.y); ctx.fillStyle = '#cba6f7'
          ctx.beginPath(); ctx.arc(p.x, p.y, 4 * this._zoom, 0, Math.PI * 2); ctx.fill()
        }
      }
    }
  ...
  _zoomBy(factor, centerX, centerY) {
    ...
    this._zoom *= factor; this._zoom = Math.max(0.1, Math.min(10, this._zoom))
  ...
  _onWheel(e) {
    if (this._destroyed) return; e.preventDefault()
    const rect = this.canvas.getBoundingClientRect()
    const sx = e.clientX - rect.left; const sy = e.clientY - rect.top
  ...
  zoom(factor) { this._zoomBy(factor) }
  ...
  fitToScreen() {
    if (this._entities.length === 0) { this._zoom = 1; this._panX = 0; this._panY = 0; this._invalidateSnapshot(); this.render(); return }
    let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity
    for (const e of this._entities) {
      ...
      else if (e.type === 'POLYGON' || e.type === 'POLYLINE') { for (const v of e.vertices) { minX = Math.min(minX, v.x); minY = Math.min(minY, v.y); maxX = Math.max(maxX, v.x); maxY = Math.max(maxY, v.y) } }
    ...
    const padding = 2; const worldW = (maxX - minX) + padding * 2; const worldH = (maxY - minY) + padding * 2
    ...
    this._zoom = Math.min(screenW / (worldW * this._gridSize), screenH / (worldH * this._gridSize))
  ...
  async _tryBackendUndo() { try { await canvasApi.undoCommand() } catch (e) { /* offline-safe */ } }
  async _tryBackendRedo() { try { await canvasApi.redoCommand() } catch (e) { /* offline-safe */ } }
    ...
    for (const e of this._entities) {
      if (e.type === 'LINE') {
        ...
        if (e.dimType === 'linear') {
          ...
          const dx = e.x2 - e.x1, dy = e.y2 - e.y1; const len = Math.hypot(dx, dy)
          if (len > 0) {
            const nx = -dy / len, ny = dx / len; const ox = nx * (e.offset || 15), oy = ny * (e.offset || 15)
            s += '0\nLINE\n8\n' + e.layer + '\n10\n' + (e.x1 + ox) + '\n20\n' + (e.y1 + oy) + '\n11\n' + (e.x2 + ox) + '\n21\n' + (e.y2 + oy) + '\n'
          }
        } else if (e.dimType === 'radius') {
          const a = e.leaderAngle || -Math.PI / 4
          s += '0\nLINE\n8\n' + e.layer + '\n10\n' + e.cx + '\n20\n' + e.cy + '\n11\n' + (e.cx + (e.radius + 15) * Math.cos(a)) + '\n21\n' + (e.cy + (e.radius + 15) * Math.sin(a)) + '\n'
        } else if (e.dimType === 'angle') {
          const r = e.arcRadius || 20
          for (let i = 0; i < 12; i++) {
            const t0 = e.angleStart + (e.angleEnd - e.angleStart) * i / 12
            const t1 = e.angleStart + (e.angleEnd - e.angleStart) * (i + 1) / 12
  ...
  addEntity(entity) {
    ...
    const cmd = this._makeAddCmd(e); this._history.execute(cmd)
    this._logCommand('ADD_ENTITY', e.id, null, { ...e })
    this._emit('entityAdded', e); this._emit('modified'); return e
  }

  removeEntity(id) {
    const idx = this._entities.findIndex(e => e.id === id)
    if (idx < 0) return false
    const entity = this._entities[idx]; const before = { ...entity }
    const cmd = this._makeRemoveCmd(entity); this._history.execute(cmd)
    this._logCommand('DELETE_ENTITY', id, before, null)
    this._emit('entityRemoved', id); this._emit('modified'); return true
  }

  getEntities() { return this._entities }

  setEntities(entities) {
    this._entities = entities.slice(); this._selectedEntity = null; this._history = new HistoryManager(60)
    this._invalidateSnapshot(); this._emit('select', null); this._emit('modified'); this.render()
  }

  clear() {
    this._entities = []; this._selectedEntity = null; this._history = new HistoryManager(60)
    this._invalidateSnapshot(); this._emit('select', null); this._emit('modified'); this.render()
  }

  updateEntity(id, props) {
    const entity = this._entities.find(e => e.id === id); if (!entity) return
    const before = deepClone(entity); Object.assign(entity, props)
    const cmd = this._makeUpdateCmd(entity, before); this._history.execute(cmd)
    this._invalidateSnapshot(); this._snap._invalidateCache(); this._emit('select', entity); this.render()
  }

  selectEntity(x, y) {
    const hit = this._hitTest(x, y); this._selectedEntity = hit
    this._emit('select', hit); this._invalidateSnapshot(); this.render(); return hit
  }

  selectById(id) {
    const entity = this._entities.find(e => e.id === id)
    if (entity) { this._selectedEntity = entity; this._emit('select', entity); this._invalidateSnapshot(); this.render() }
    return entity || null
  }

  moveSelected(dx, dy) {
    if (!this._selectedEntity) return
    const before = deepClone(this._selectedEntity); this._moveEntity(this._selectedEntity, dx, dy)
    const after = deepClone(this._selectedEntity)
    const cmd = this._makeMoveCmd(this._selectedEntity, before); this._history.execute(cmd)
    this._logCommand('MOVE_ENTITY', this._selectedEntity.id, before, after)
    this._invalidateSnapshot(); this._emit('modified'); this.render()
  }

  deleteSelected() {
    if (!this._selectedEntity) return
    const entity = this._selectedEntity; const before = { ...entity }
    const cmd = this._makeRemoveCmd(entity); this._history.execute(cmd)
    this._logCommand('DELETE_ENTITY', entity.id, before, null)
    this._emit('entityRemoved', entity.id); this._emit('modified'); this.render()
  }
  ...
  setTool(tool) {
    ...
    const prev = this._tool; this._tool = tool; this._editState = null
    if (tool !== 'polygon') { this._isDrawing = false; this._drawingEntity = null; this._drawingPhase = 0; if (this._polygonHintEl) this._polygonHintEl.style.display = 'none' }
    if (tool !== 'polyline') { this._isDrawing = false; this._drawingEntity = null; this._drawingPhase = 0; if (this._polylineHintEl) this._polylineHintEl.style.display = 'none' }
    if (tool !== 'dim') { this._dimState = null; this._dimPreview = null; this._dimLine1 = null }
    if (tool !== 'text' && this._textInputOverlay) { this._textInputOverlay.style.display = 'none' }
    if (tool === 'select' || tool === 'pan') { /* keep selection */ }
    else if (!isEditCommand(tool)) { this._selectedEntity = null; this._emit('select', null) }
  ...
  setActiveLayer(layerId) {
    ...
    if (layer) { this._layerColors[layerId] = layer.color; this._layerVisibility[layerId] = layer.visible }
  }
  ...
  addLayer(name, color) {
    const id = this._nextId(); const layer = { id, name, color: color || '#a6e3a1', visible: true, locked: false }
    this._layers.push(layer); this._layerVisibility[id] = true; this._layerColors[id] = layer.color; this._layerLocked[id] = false
    this._invalidateSnapshot(); this._emit('modified'); return { ...layer }
  }

  removeLayer(id) {
    if (this._layers.length <= 1) return false
    const idx = this._layers.findIndex(l => l.id === id); if (idx < 0) return false
    this._layers.splice(idx, 1); delete this._layerVisibility[id]; delete this._layerColors[id]; delete this._layerLocked[id]
    for (const e of this._entities) { if (e.layer === id) e.layer = '0' }
    if (this._activeLayer === id) this.setActiveLayer('0')
    this._invalidateSnapshot(); this._emit('modified'); return true
  }

  setLayerLocked(id, locked) {
    const layer = this._layers.find(l => l.id === id); if (!layer) return
    layer.locked = !!locked; this._layerLocked[id] = !!locked; this._emit('modified')
  }

  getLayerLocked(id) { return !!this._layerLocked[id] }

  toggleSnap() { this._snap.snapToGrid = !this._snap.snapToGrid; this._invalidateSnapshot(); this.render() }
  toggleOrtho() { this._ortho = !this._ortho; if (!this._ortho) this._snapOrigin = null; this._invalidateSnapshot(); this.render() }

  getSnapState() { return { snapToGrid: this._snap.snapToGrid, ortho: this._ortho } }
  getZoom() { return this._zoom }
  setSnapshotMode(enable) { this._snapshotMode = enable; if (!enable) this._snapshotValid = false }
  ...
  setLayerColor(layerId, color) {
    ...
    for (const e of this._entities) { if (e.layer === layerId) e.color = color }
    this._invalidateSnapshot(); this.render()
  }
  ...
  on(event, callback) {
    ...
    return () => { const h = this._handlers[event]; if (h) this._handlers[event] = h.filter(fn => fn !== callback) }
  }
  ...
  resize() { this._setupSize(); this.render() }

  destroy() {
    this._destroyed = true; this._unbindEvents(); this._removeOverlays()
    this._entities = []; this._selectedEntity = null; this._drawingEntity = null; this._handlers = {}
  }

  exportJSON() { return JSON.stringify({ version: '1.0', entities: this._entities, nextId: this._idCounter }) }
  ...
  importJSON(json) {
    try {
      ...
      this._selectedEntity = null; this._history = new HistoryManager(60)
      this._invalidateSnapshot(); this._emit('select', null); this._emit('modified'); this.render()
      return true
